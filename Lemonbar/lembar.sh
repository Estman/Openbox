...
#while read -r line ; do
 # case $line in
    ...
    # power
  #    power="%{A2:sudo poweroff:}%{A3:sudo reboot:}$PADDING${line#?}$PADDING%{A}%{A}"
   #   ;;
...
power() {
       poweroff=$(sudo poweroff)
       reboot=$(sudo reboot)
       echo "%{A:/home/david/.config/lemonbar/poweroff.sh:}%{A2}  %{A1}$poweroff%{A}  %{A2}RE %{A1}$reboot"
}

# Print the power

#while true; do
        #echo "%{A:/home/david/.config/lemonbar/poweroff.sh:}%{T2}  %{T1}$poweroff%{A}  %{T2}RE %{T1}$reboot"
        #sleep 1;
        #power="%{A2:sudo poweroff:}%{A3:sudo reboot:}$PADDING${line#?}$PADDING%{A}%{A}"
#done
...
netstatus() {
    curethstat=`cat /sys/class/net/enp3s0/operstate`
...
while :; do
    ...
    echo "%{B#DDecf0f1}%{F#2c2c2c}%{T2}%{l}    $(groups)    %{T1}$(curwin) %{r}%{F#2C2C2C}$(ircnotify) $(netstatus) $(volume) $(clock) %{A2}$(curwin) "
    sleep .2
